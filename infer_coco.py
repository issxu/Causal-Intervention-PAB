...
from models.model_search_coco import Search
...
def infer_topk_images_with_itm(model, tokenizer, queries, image_loader, device, config, topk=10):
    """
    ...
    text_embed = model.get_text_embeds(text_inputs.input_ids, text_inputs.attention_mask)
    ...
    for images, img_ids in image_loader:
        ...
        image_embed, _ = model.get_vision_embeds(images)
...
def main(args, config):
    ...
    output_lines = []
    topk_image_files = infer_topk_images_with_itm(model, tokenizer, queries, image_loader, device, config, topk=10)
