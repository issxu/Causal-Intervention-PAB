...
def evaluation_itc(model, data_loader, tokenizer, device, config):
    model.eval()
    print('Computing features for evaluation with Query Expansion...')
    start_time = time.time()

    # texts_grouped ????????, e.g., [['capA1', 'capA2'], ['capB1', 'capB2']]
    texts_grouped = data_loader.dataset.text
    
    # ??? ????:?????? ???
    avg_text_feats = []
    # ????????? text_embeds ? text_atts ?? ITM ??
    # ???? ITM ??????? caption (????? embed)
    # ???????????,?? ITC ????????
    
    for caption_group in texts_grouped:
        # 1. ????????????? (e.g., 5?) caption
        text_input = tokenizer(caption_group, padding='max_length', truncation=True, 
        ...
        text_feat = F.normalize(text_feat, dim=-1) # ??? [5, embed_dim]

        # 2. ?????
        avg_feat = torch.mean(text_feat, dim=0) # ???? [embed_dim]
        avg_text_feats.append(avg_feat)
    
    # 3. ???????????????????
    text_feats = torch.stack(avg_text_feats, dim=0) # ??? [num_queries, embed_dim]
    
    # --- ITM ????????? ---
    # ?????? evaluation_itm ???,???????????????
    # ???????:??????????caption??ITM??
    # ???????????,??????????
    first_captions = [group[0] for group in texts_grouped]
    text_input_itm = tokenizer(first_captions, padding='max_length', truncation=True,
                               max_length=config['max_tokens'], return_tensors="pt").to(device)
    text_embeds = model.get_text_embeds(text_input_itm.input_ids, text_input_itm.attention_mask)
    text_atts = text_input_itm.attention_mask
