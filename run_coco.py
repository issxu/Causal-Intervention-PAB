...
def run(args):
    if args.task in ['baseline', 'cmp', 'my', 'my_test', 'my_more', 'my_more_more_coco']:
        args.config = 'configs/' + args.task + '.yaml'
        print(args.task, args.config)

        dist_launch = get_dist_launch(args)
        os.system(
            f"{dist_launch} Search_coco.py --config {args.config} --task {args.task} --output_dir {args.output_dir} "
